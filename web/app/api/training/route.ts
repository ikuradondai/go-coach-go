...
import { EXERCISE_CATALOG, toExerciseView } from '@/domain/exercises';
...
export async function GET() {
  ...
  const result = await getDb().prepare(
    `SELECT payload_json FROM exercises WHERE status = 'published' AND id IN (${ids.map(() => '?').join(',')}) ORDER BY ordinal`
  ).bind(...ids).all<{ payload_json: string }>();
