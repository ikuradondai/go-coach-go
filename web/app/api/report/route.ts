...
type AttemptRow = { group_correct: number; reasons_correct: number; error_tag: string | null };
...
export async function GET(request: Request) {
  ...
  const rows = (await getDb().prepare(`
    SELECT group_correct, reasons_correct, error_tag FROM attempts
  ...
  const accuracy = attemptCount ? Math.round(((groupPoints * 0.6 + reasonPoints * 0.4) / attemptCount) * 100) : 0;
