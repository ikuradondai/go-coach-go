...
export default function SgfAdmin() {
  ...
  useEffect(() => { void refresh().catch(() => setError('SGF管理データを読み込めませんでした。')); }, []);
  useEffect(() => {
  ...
  const analyze = async (candidate: PositionCandidate) => {
    ...
    try {
      ...
      setMessage(`${candidate.moveNumber}手目をKataGoで検証しました。`); await refresh(selectedImportId);
    } catch (caught) { setError(caught instanceof Error ? caught.message : 'KataGo解析に失敗しました。'); await refresh(selectedImportId); }
    finally { setBusy(null); }
  };

  return <section className="sgf-workspace" id="sgf-workspace">
    <div className="workspace-heading"><div><span>CONTENT PIPELINE</span><h2>SGFから検証済み局面を作る</h2><p>棋譜を取り込み、主変化の任意の手数を選び、KataGoの候補手・勝率・目差で教材化前に確認します。</p></div><div className={`engine-badge ${data.katagoConfigured ? 'ready' : 'offline'}`}><i />KataGo {data.katagoConfigured ? '接続設定済み' : '接続先未設定'}</div></div>
    ...
    <div className="candidate-section"><div className="candidate-heading"><div><span>VALIDATION QUEUE</span><h3>検証候補</h3></div><label>探索量 <select value={visits} onChange={(event) => setVisits(Number(event.target.value))}><option value="100">100 visits（確認用）</option><option value="400">400 visits（標準）</option><option value="1200">1,200 visits（精査）</option></select></label></div>
      <div className="candidate-list">{!data.candidates.length && <p className="empty-candidates">選択した局面がここに並びます。</p>}{data.candidates.map((candidate) => { const result = candidate.analysis?.result; const best = result?.moveInfos?.[0]; return <article key={candidate.id}><div className="candidate-board"><AdminGoBoard position={candidate.position} /></div><div className="candidate-info"><span className={`candidate-status ${candidate.status}`}>{STATUS_LABEL[candidate.status]}</span><h4>{data.imports.find((item) => item.id === candidate.importId)?.fileName ?? 'SGF'}・{candidate.moveNumber}手目</h4><p>次は{candidate.toPlay === 'black' ? '黒' : '白'}番</p>{result && <div className="analysis-summary"><div><span>最善手</span><strong>{best?.move ?? '—'}</strong></div><div><span>勝率</span><strong>{best ? `${(best.winrate * 100).toFixed(1)}%` : '—'}</strong></div><div><span>目差</span><strong>{best ? `${best.scoreLead > 0 ? '+' : ''}${best.scoreLead.toFixed(1)}` : '—'}</strong></div><div><span>探索数</span><strong>{result.rootInfo.visits.toLocaleString()}</strong></div></div>}{candidate.analysis?.error && <p className="analysis-error">{candidate.analysis.error}</p>}<button type="button" disabled={!data.katagoConfigured || busy === candidate.id} onClick={() => void analyze(candidate)}>{busy === candidate.id ? '解析中…' : result ? '再解析する' : 'KataGoで検証'}</button></div></article>; })}</div>
    </div>
  </section>;
}
